...
openai_client = AsyncOpenAI(api_key=OCR_LLM_API_KEY, base_url=OCR_LLM_BASE_URL)
...
QUESTION_OCR_PROMPT = """You are a professional OCR system specialized in high school mathematics exams.
...
6. Do NOT guess missing parts.
7. Do NOT repeat any content.
8. If the question is incomplete or unclear, extract only the visible part.
9. If the question contains any printed diagrams, graphs, geometric figures, or illustrations, set the has_image field to true. If there are no images, set it to false.
10. Output VALID JSON only. No extra text before or after JSON.
...
def encode_image_bytes_to_data_url(content_type: str, data: bytes) -> str:
    encoded = base64.b64encode(data).decode("ascii")
    return f"data:{content_type};base64,{encoded}"


def encode_upload_file_to_data_url(upload: UploadFile, data: bytes) -> str:
    content_type = upload.content_type or "application/octet-stream"
    return encode_image_bytes_to_data_url(content_type, data)
...
def parse_ocr_json(raw: str) -> dict[str, Any] | list[Any]:
...
async def ocr_chat_completion(prompt: str, image_data_url: str) -> str:
    try:
...
async def paper_ocr_from_data_url(image_data_url: str) -> tuple[str, dict[str, Any] | list[Any]]:
    raw_text = await ocr_chat_completion(PAPER_OCR_PROMPT, image_data_url)
    payload = parse_ocr_json(raw_text)
    return raw_text, payload

async def question_ocr_from_data_url(image_data_url: str) -> tuple[str, dict[str, Any] | list[Any]]:
    raw_text = await ocr_chat_completion(QUESTION_OCR_PROMPT, image_data_url)
    payload = parse_ocr_json(raw_text)
    return raw_text, payload

async def answer_ocr_from_data_url(image_data_url: str) -> str:
    return await ocr_chat_completion(ANSWER_OCR_PROMPT, image_data_url)
